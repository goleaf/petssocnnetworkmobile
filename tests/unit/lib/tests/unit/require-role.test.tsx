...
// Ensure fetchSession is a jest.fn in this test file
...
// auth-server is mocked globally in jest.setup with jest.fn exports
...
describe.skip("RequireRole", () => {
  ...
  describe("SSR 403 on no session", () => {
    ...
    it("should redirect when session is null", async () => {
      ...
      ;(authServer.fetchSession as unknown as jest.Mock).mockResolvedValue(null)
    ...
    it("should redirect when user is null", async () => {
      const { RequireRole } = await import("@/components/admin/require-role")
      ;(authServer.fetchSession as unknown as jest.Mock).mockResolvedValue({
        session: mockSession,
        user: null,
      })
