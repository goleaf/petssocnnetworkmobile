  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 max-w-7xl">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 sm:mb-6">Search & Discover</h1>
        <div className="flex flex-col gap-3 sm:gap-4 mb-4 sm:mb-6 relative">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[calc(100vw-2rem)] sm:w-80 max-h-[80vh] overflow-y-auto">
                ...
                {["dog", "cat", "bird", "rabbit", "hamster", "fish", "other"].map((species) => {
                  ...
                  const Icon = animalConfig?.lucideIcon
        ...
        {!query && popularSearches.length > 0 && (
          <div className="mb-4 sm:mb-6">
            <div className="flex items-center gap-2 mb-2 sm:mb-3">
              ...
              <span className="text-xs sm:text-sm font-semibold text-muted-foreground">Popular Searches</span>
            </div>
            <div className="flex flex-wrap gap-2">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-3 sm:grid-cols-6 gap-1">
          <TabsTrigger value="all" className="text-xs sm:text-sm">All ({totalResults})</TabsTrigger>
          <TabsTrigger value="users" className="text-xs sm:text-sm">
            <User className="h-3 w-3 sm:h-4 sm:w-4 mr-1 text-blue-500" />
            <span className="hidden sm:inline">Users ({results.users.length})</span>
            <span className="sm:hidden">{results.users.length}</span>
          </TabsTrigger>
          <TabsTrigger value="pets" className="text-xs sm:text-sm">
            <PawPrint className="h-3 w-3 sm:h-4 sm:w-4 mr-1 text-orange-500" />
            <span className="hidden sm:inline">Pets ({results.pets.length})</span>
            <span className="sm:hidden">{results.pets.length}</span>
          </TabsTrigger>
          <TabsTrigger value="blogs" className="text-xs sm:text-sm">
            <FileText className="h-3 w-3 sm:h-4 sm:w-4 mr-1 text-green-500" />
            <span className="hidden sm:inline">Blogs ({results.blogs.length})</span>
            <span className="sm:hidden">{results.blogs.length}</span>
          </TabsTrigger>
          <TabsTrigger value="wiki" className="text-xs sm:text-sm">
            <BookOpen className="h-3 w-3 sm:h-4 sm:w-4 mr-1 text-purple-500" />
            <span className="hidden sm:inline">Wiki ({results.wiki.length})</span>
            <span className="sm:hidden">{results.wiki.length}</span>
          </TabsTrigger>
          <TabsTrigger value="hashtags" className="text-xs sm:text-sm">
            <Hash className="h-3 w-3 sm:h-4 sm:w-4 mr-1 text-pink-500" />
            <span className="hidden sm:inline">Tags ({results.hashtags.length})</span>
            <span className="sm:hidden">{results.hashtags.length}</span>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="pets" className="mt-4 sm:mt-6">
          {isLoading ? (
            <div className="flex justify-center py-12">
              <LoadingSpinner />
            </div>
            ...
            <Card>
              <CardContent className="p-8 sm:p-12 text-center">
                <PawPrint className="h-10 w-10 sm:h-12 sm:w-12 mx-auto text-muted-foreground mb-4" />
                <p className="text-sm sm:text-base text-muted-foreground">No pets found</p>
              </CardContent>
            </Card>
          ) : (
            <>
          ...
          {isLoading ? (
            ...
            <Card>
              <CardContent className="p-8 sm:p-12 text-center space-y-4">
                <FileText className="h-10 w-10 sm:h-12 sm:w-12 mx-auto text-muted-foreground/50" />
                <div className="space-y-2">
                  <p className="text-sm sm:text-base text-muted-foreground font-medium">No blog posts found</p>
                  <p className="text-xs sm:text-sm text-muted-foreground/80">Try adjusting your search terms or filters</p>
                </div>
              </CardContent>
            </Card>
          ) : (
        ...
        <TabsContent value="wiki" className="mt-4 sm:mt-6">
          {isLoading ? (
            <div className="flex justify-center py-12">
              <LoadingSpinner />
            </div>
            ...
            <Card>
              <CardContent className="p-8 sm:p-12 text-center">
                ...
                <p className="text-sm sm:text-base text-muted-foreground">No wiki articles found</p>
              </CardContent>
            </Card>
          ) : (
            <>
        ...
        <TabsContent value="hashtags" className="mt-4 sm:mt-6">
          {isLoading ? (
            ...
            <Card>
              <CardContent className="p-8 sm:p-12 text-center">
                <Hash className="h-10 w-10 sm:h-12 sm:w-12 mx-auto text-muted-foreground mb-4" />
                <p className="text-sm sm:text-base text-muted-foreground">No hashtags found</p>
              </CardContent>
            </Card>
          ) : (
            <div className="flex flex-wrap gap-2">
              {results.hashtags.map((tag) => (
                <Link key={tag} href={`/search?q=${encodeURIComponent(`#${tag}`)}&tab=blogs`}>
                  <Badge
                    variant="secondary"
                    className="text-sm sm:text-base px-3 sm:px-4 py-1.5 sm:py-2 cursor-pointer hover:bg-primary hover:text-primary-foreground transition-colors"
                  >
                    #{tag}
                  </Badge>
                </Link>
...
function PetCard({ pet, query }: { pet: Pet; query: string }) {
  ...
  const animalConfig = ANIMAL_TYPES.find(a => a.value === pet.species)
  const IconComponent = animalConfig?.lucideIcon
              ...
              <div className="flex items-center gap-2 flex-wrap mb-1">
                {IconComponent && (
                  <Badge variant="outline" className="text-xs capitalize">
                    <IconComponent className={`h-3 w-3 mr-1 ${animalConfig?.color || ""}`} />
                    {animalConfig ? animalConfig.label : pet.species}
                  </Badge>
                )}
  ...
  return (
    <Link href={`/wiki/${article.slug}`}>
      <Card className="hover:bg-muted/50 transition-colors cursor-pointer">
        <CardContent className="p-3 sm:p-4">
          <div className="flex items-start gap-3">
            <BookOpen className="h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground mt-1 flex-shrink-0" />
