...
import { Heart, MessageCircle, ArrowLeft, Send, Reply, Edit2, Trash2, X, Check, Smile, MoreHorizontal } from "lucide-react"
  ...
  return (
    <div className="container mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl">
      ...
      <Card className="overflow-hidden p-0">
        ...
        <CardHeader className="space-y-4 px-3 sm:px-6 pt-4 sm:pt-6">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex-1 min-w-0">
              <Link href={getPetUrlFromPet(pet, author.username)} className="font-semibold hover:underline block truncate">
                {pet.name}
              </Link>
        ...
        <CardContent className="prose prose-lg max-w-none px-3 sm:px-6 pb-4 sm:pb-6">
      ...
      <Card className="mt-6">
        <CardHeaderWithIcon
          title="Comments"
          description={`${totalCommentsCount} ${totalCommentsCount === 1 ? "Comment" : "Comments"}`}
          icon={MessageCircle}
        />
        <CardContent className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          {/* Add Comment */}
          {currentUser && (
            <div className="flex gap-3 sm:gap-4 pb-4 sm:pb-6 border-b">
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {replyingTo === comment.id && currentUser && (
                        <div className="mt-3 sm:mt-4 ml-1 sm:ml-4 pl-1 sm:pl-4 border-l-2 border-primary/30 space-y-2 sm:space-y-3">
                          <div className="flex gap-2 sm:gap-3">
                            <Avatar className="h-8 w-8 sm:h-9 sm:w-9 ring-2 ring-border flex-shrink-0">
                              <AvatarImage src={currentUser.avatar || "/placeholder.svg"} alt={currentUser.fullName} />
                      ...
                      {/* Replies */}
                      {replies.length > 0 && (
                        <div className="mt-3 sm:mt-5 ml-1 sm:ml-4 pl-1 sm:pl-5 space-y-3 sm:space-y-4 border-l-2 border-primary/20">
                          {replies.map((reply) => {
                              ...
                              <div key={reply.id} className="group/reply">
                                <div className="flex gap-2 sm:gap-3">
                                  <Link href={`/profile/${replyUser?.username}`} className="flex-shrink-0">
                                    <Avatar className="h-8 w-8 sm:h-9 sm:w-9 ring-2 ring-border hover:ring-primary/50 transition-all">
                                      <AvatarImage
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="bg-muted/40 rounded-lg p-2.5 sm:p-3.5 border border-border/50 hover:border-border transition-all">
                                      <div className="flex items-start justify-between mb-2 gap-2">
                                        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap min-w-0">
                                          <Link
                                            href={`/profile/${replyUser?.username}`}
                                            className="font-semibold text-xs hover:text-primary transition-colors truncate"
                                          >
                                            {replyUser?.fullName}
                                          </Link>
                                          ...
                                          <span className="text-[10px] text-muted-foreground whitespace-nowrap">replied to</span>
                      <Link
                        href={`/profile/${commentUser?.username}`}
                                          ...
                                          <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                              <Button
                                                variant="ghost"
                                                size="icon"
                                                className="h-7 w-7 opacity-0 group-hover/reply:opacity-100 transition-opacity flex-shrink-0"
                                              >
          ...
          {topLevelComments.length === 0 && (
            <div className="text-center py-8 sm:py-12">
              <MessageCircle className="h-8 w-8 sm:h-12 sm:w-12 mx-auto text-muted-foreground/50 mb-2 sm:mb-3" />
