...
export default function NewPagesQueuePage() {
  const { user } = useAuth();
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      setSuccessMessage("New page approved and published successfully.");
      // Trigger queue refresh by forcing re-render
      router.refresh();
    } catch (error) {
  ...
  const submitReject = async () => {
    ...
    try {
      ...
      setSuccessMessage("New page rejected successfully.");
  ...
  const handleBulkOperation = async (
    ...
    try {
      ...
      if (failureCount === 0) {
        setSuccessMessage(
          `Successfully ${operation}d ${successCount} new page${successCount > 1 ? "s" : ""}.`
        );
      } else {
        setErrorMessage(
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reject New Page</DialogTitle>
            <DialogDescription>
              Please provide a reason for rejecting this new page. The user will be
              notified.
            </DialogDescription>
          </DialogHeader>
