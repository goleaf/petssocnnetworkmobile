...
export default function ImageReviewsQueuePage() {
  const { user } = useAuth();
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      setSuccessMessage("Image content approved and published successfully.");
      router.refresh();
    } catch (error) {
  ...
  const submitReject = async () => {
    ...
    try {
      ...
      setSuccessMessage("Image content rejected successfully.");
  ...
  const handleBulkOperation = async (
    ...
    try {
      ...
      if (failureCount === 0) {
        setSuccessMessage(
          `Successfully ${operation}d ${successCount} image review${successCount > 1 ? "s" : ""}.`
        );
      } else {
        setErrorMessage(
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reject Image Content</DialogTitle>
            <DialogDescription>
              Please provide a reason for rejecting this content with images. The user
              will be notified.
            </DialogDescription>
          </DialogHeader>
