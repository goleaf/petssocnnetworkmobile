...
export default function COIEditsQueuePage() {
  const { user } = useAuth();
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      setSuccessMessage("COI edit approved and published successfully.");
      router.refresh();
    } catch (error) {
  ...
  const submitReject = async () => {
    ...
    try {
      ...
      setSuccessMessage("COI edit rejected successfully.");
  ...
  const handleBulkOperation = async (
    ...
    try {
      ...
      if (failureCount === 0) {
        setSuccessMessage(
          `Successfully ${operation}d ${successCount} COI edit${successCount > 1 ? "s" : ""}.`
        );
      } else {
        setErrorMessage(
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reject COI Edit</DialogTitle>
            <DialogDescription>
              Please provide a reason for rejecting this edit with potential conflict
              of interest. The user will be notified.
            </DialogDescription>
          </DialogHeader>
