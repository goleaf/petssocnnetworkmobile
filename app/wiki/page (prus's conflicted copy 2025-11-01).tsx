...
export default function WikiPage() {
  ...
  // Create species icons mapping from ANIMAL_TYPES
  const speciesIcons: Record<string, any> = useMemo(() => {
    const mapping: Record<string, any> = {}
    ANIMAL_TYPES.forEach((animal) => {
      mapping[animal.value] = animal.lucideIcon
    })
    // Add fallback for any species not in ANIMAL_TYPES
    mapping['other'] = PawPrint
    return mapping
  }, [])

  // Get all unique species from articles that exist in ANIMAL_TYPES
  const allSpecies = useMemo(() => {
    const speciesSet = new Set<string>()
    const validSpecies = new Set(ANIMAL_TYPES.map(a => a.value))
    articles.forEach((article) => {
      article.species?.forEach((s) => {
        if (validSpecies.has(s)) {
          speciesSet.add(s)
        }
      })
    })
    return Array.from(speciesSet).sort((a, b) => {
      const aIndex = ANIMAL_TYPES.findIndex(animal => animal.value === a)
      const bIndex = ANIMAL_TYPES.findIndex(animal => animal.value === b)
    ...
    return (
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 max-w-7xl">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2">Pet Care Wiki</h1>
    ...
    <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 max-w-7xl">
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xl sm:text-2xl font-bold truncate">{totalArticles}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xl sm:text-2xl font-bold truncate">{totalViews.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xl sm:text-2xl font-bold truncate">{totalLikes.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xl sm:text-2xl font-bold truncate">{categories.length - 1}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            {featuredArticles.map((article) => {
              ...
              return (
                <Link key={article.id} href={`/wiki/${article.slug}`}>
                  <Card className="hover:shadow-lg transition-all duration-200 cursor-pointer h-full overflow-hidden p-0 group">
                    {article.coverImage && (
                    ...
                    <CardHeader className="px-4 sm:px-6 pt-4 sm:pt-6">
                      ...
                      <CardTitle className="line-clamp-2 text-base sm:text-lg">{article.title}</CardTitle>
                    </CardHeader>
                    <CardContent className="px-4 sm:px-6 pb-4 sm:pb-6">
                      ...
                      <div className="flex items-center gap-3 sm:gap-4 text-xs sm:text-sm text-muted-foreground flex-wrap">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="truncate">{article.likes.length}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Clock className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                          <span className="truncate">{calculateReadingTime(article.content)} min</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Link key={article.id} href={`/wiki/${article.slug}`}>
                <Card className="hover:shadow-lg transition-all duration-200 cursor-pointer h-full group">
                  ...
                  <CardHeader className="p-3 sm:p-4">
                    <CardTitle className="text-xs sm:text-sm line-clamp-2">{article.title}</CardTitle>
                  </CardHeader>
                </Card>
              </Link>
            ))}
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <Select value={selectedSpecies} onValueChange={setSelectedSpecies}>
          <SelectTrigger className="w-full sm:w-[160px] md:w-[180px] h-9 sm:h-10 text-sm sm:text-base">
            <SelectValue>
                ...
                {selectedSpecies === "all" ? (
                  ...
                  <>
                    {(() => {
                      ...
                      const animalConfig = ANIMAL_TYPES.find(a => a.value === selectedSpecies)
                      const label = animalConfig ? animalConfig.label : selectedSpecies.charAt(0).toUpperCase() + selectedSpecies.slice(1)
      ...
      <Tabs value={selectedCategory} onValueChange={handleCategoryChange} className="w-full mb-4 sm:mb-6">
        {/* Combined Category and Subcategory Filters */}
        <div className="mb-4 sm:mb-6">
          <TabsList className="grid w-full grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-1 sm:gap-2 mb-3 sm:mb-4 h-auto p-1">
            {categories.map((category) => (
              ...
              >
                <category.icon className={`h-3 w-3 sm:h-4 sm:w-4 ${category.color} flex-shrink-0`} />
          ...
          {selectedCategory !== "all" && currentSubcategories.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                <span className="hidden sm:inline">All {categories.find((c) => c.value === selectedCategory)?.label}</span>
                ...
                <Link key={article.id} href={`/wiki/${article.slug}`}>
                  <Card className="hover:shadow-lg transition-all duration-200 cursor-pointer h-full overflow-hidden p-0 group">
                    ...
                    <CardHeader className="px-4 sm:px-6 pt-4 sm:pt-6">
                      <div className="flex items-start justify-between gap-2 mb-2 flex-wrap">
                        ...
                        {article.species && article.species.length > 0 && (() => {
                          const species = article.species[0]
                          const animalConfig = ANIMAL_TYPES.find(a => a.value === species)
                          ...
                          return (
                            <Badge variant="outline" className="capitalize text-xs sm:text-sm">
                              {IconComponent && (
                                <IconComponent className={`h-3 w-3 mr-1 ${animalConfig?.color || ""} flex-shrink-0`} />
                              )}
                              <span className="truncate">{label}</span>
                            </Badge>
                          )
                    ...
                    <CardContent className="px-4 sm:px-6 pb-4 sm:pb-6">
                      ...
                      <div className="flex items-center gap-3 sm:gap-4 mt-3 sm:mt-4 text-xs sm:text-sm text-muted-foreground flex-wrap">
                        <div className="flex items-center gap-1">
                          <Eye className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                          <span className="truncate">{article.views}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                          <Clock className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                          <span className="truncate">{calculateReadingTime(article.content)} min read</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              {paginatedArticles.length === 0 ? (
                <Card>
                  <CardContent className="p-8 sm:p-12 text-center space-y-3 sm:space-y-4">
                    ...
                    <p className="text-sm sm:text-base text-muted-foreground">No articles found in this category</p>
                  </CardContent>
                </Card>
              ) : totalPages > 1 && (
